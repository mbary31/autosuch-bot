...
def parse_price_eur(text: str):
    """Extrahiert Preis wie '12.300 €' -> 12300"""
    t = text.replace("\xa0", " ")
    m = PRICE_RE.search(t)
    if not m:
        return None
...
def parse_first_registration_year(text: str):
    """Sucht Erstzulassung (EZ) Jahr auf der Detailseite."""
    t = text.replace("\xa0", " ")

    for key in ["Erstzulassung", "EZ", "First registration", "Erst-Zulassung"]:
...
def looks_like_blocked(html: str) -> bool:
    h = html.lower()
    needles = [
        "captcha", "access denied", "forbidden", "cloudflare",
        "cookie", "consent", "bitte bestätige", "automated",
        "unusual traffic", "datenschutz"
    ]
    return any(n in h for n in needles)


def validate_listing_by_details(url: str, max_price: int, min_year: int) -> bool:
    """Lädt Detailseite und prüft Preis + EZ-Jahr. Nur wenn OK -> True."""
    code, html = fetch(url, timeout=25)
    if code != 200:
        return False
...
def parse_set(args: list[str]):
    ...
    /set audi a4 avant 76133 100 9000 2010
    """
    ...
    q = " ".join(t.strip() for t in query_tokens if t.strip()).lower().strip()
    if not q:
        return None
...
async def run_checks_for_chat(chat_id: str, app: Application, verbose: bool = False):
    ...
    for cfg in cfgs:
        ...
        for u in urls:
            ...
            # ✅ Nur senden, wenn Detailseite wirklich passt
            if not validate_listing_by_details(u, max_price=max_price, min_year=min_year):
                continue
        ...
        if to_send:
            ...
            if verbose:
                await app.bot.send_message(chat_id=int(chat_id), text=f"ℹ️ Keine neuen Links gerade.\n({', '.join(diag)})")
